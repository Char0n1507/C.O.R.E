...
DB_PATH = os.path.join(
    os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "data", "soc_agent.db"
)
...
class PDFReport(FPDF):
    def header(self):
        ...
        self.cell(
            0, 10, "C.O.R.E AI SOC - Executive Threat Report", border=False, align="C"
        )
        self.ln(20)
...
def generate_daily_report():
    ...
    try:
        ...
        if not df.empty and "risk_score" in df.columns:
            df["risk_score"] = pd.to_numeric(df["risk_score"], errors="coerce").fillna(
                0
            )
        ...
        if not df.empty and "mitre_tactic" in df.columns:
            tactics = (
                df[(df["mitre_tactic"] != "Unknown") & (df["mitre_tactic"].notna())][
                    "mitre_tactic"
                ]
                .value_counts()
                .to_dict()
        ...
        pdf.cell(
            w=0,
            h=10,
            text=f"Report Generated: {date_str}",
            new_x="LMARGIN",
            new_y="NEXT",
        )
        pdf.ln(5)
        ...
        if tactics:
            ...
            pdf.cell(
                w=0,
                h=10,
                text="MITRE ATT&CK Kill-Chain Breakthrough",
                new_x="LMARGIN",
                new_y="NEXT",
            )
            pdf.set_font("helvetica", "", 12)
        ...
        pdf.set_font("helvetica", "B", 14)
        pdf.cell(
            w=0, h=10, text="Top 5 Critical Threats", new_x="LMARGIN", new_y="NEXT"
        )
        ...
        if critical > 0:
            ...
            pdf.cell(
                w=0,
                h=10,
                text="No critical threats detected.",
                new_x="LMARGIN",
                new_y="NEXT",
            )

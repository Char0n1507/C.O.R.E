...
class EmailMonitor:
    ...
    async def run(self):
        ...
        if self.server == "imap.example.com":
            print(
                "[*] Email Monitor running in DEMO mode reading from test_emails.jsonl"
            )
            ...
            with IMAPClient(self.server, timeout=10) as client:
                ...
                for msg_id, data in response.items():
                    ...
                    sender = (
                        ", ".join(
                            [
                                f"{e[0]} <{e[1]}>" if e[0] else e[1]
                                for e in parsed_mail.from_
                            ]
                        )
                        if hasattr(parsed_mail, "from_")
                        else "Unknown"
                    )
                    ...
                    body = (
                        ...
                        else (parsed_mail.text_html[0] if parsed_mail.text_html else "")
                    )

...
class EnterpriseEmailMonitor:
    ...
    def _acquire_token(self):
        ...
        if "access_token" in result:
            ...
            raise Exception(
                f"Failed to acquire MS Graph token: {result.get('error_description')}"
            )

    def start(self):
        if not all([self.tenant_id, self.client_id, self.client_secret, self.user_id]):
            print(
                "[!] Skipping Enterprise Email Monitor (Missing MS Graph Credentials)"
            )
            return
    ...
    def _check_inbox(self, last_check_time):
        ...
        for msg in messages:
            ...
            print(
                f"[*] Enterprise EmailMonitor Ingested: {subject} from {sender_address}"
            )
